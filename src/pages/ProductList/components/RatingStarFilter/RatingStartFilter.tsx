...
export default function RatingStartFilter({ queryConfig }: Props) {
  const { rating_filter } = queryConfig
  const navigate = useNavigate()

  const handleStarFilter = (ratingFilter: number) => {
    navigate({
      pathname: path.home,
      ...
      {Array(5)
        ...
        .map((_, index) => (
          <li className=' pl-2' key={index}>
            <button
              className={classNames('flex items-center rounded-xl py-1 px-3 text-sm', {
                'bg-gray-200': 5 - index === Number(rating_filter)
              })}
